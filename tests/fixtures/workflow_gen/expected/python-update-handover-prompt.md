...
**Project root for all `cyclopsctl tasks` commands:** `{ROOT}` (`--project-root {ROOT}`).
...
```bash
cyclopsctl tasks set-status --id=<current-task-id> --status=done --project-root {ROOT}
```
...
```bash
cyclopsctl tasks list pending --format json --project-root {ROOT}
```

From the pending list, pick the task with the **smallest numeric id**. Load full details:

```bash
cyclopsctl tasks show <next-task-id> --format json --project-root {ROOT}
```
