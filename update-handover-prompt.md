...
**Project root for all `cyclopsctl tasks` commands:** run from the project directory (defaults to cwd) or pass `--project-root`. Use **CLI only**.
...
```bash
cyclopsctl tasks set-status --id=<current-task-id> --status=done
```
...
```bash
cyclopsctl tasks list pending --format json
```

From the pending list, pick the task with the **smallest numeric id**. Load full details:

```bash
cyclopsctl tasks show <next-task-id> --format json
```
...
- `## Verification` with `python -m pytest`
