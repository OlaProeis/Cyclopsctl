...
- `AgentRunError` carries `phase` and optional `diagnostic_detail` (display-only; from `runner.extract_run_error_detail` / `run.conversation_json()`); `cli._log_and_exit_agent_run` prints an actionable report (result detail, optional `context:` line, resolved local transcript path via `failure_report.encode_project_slug` + agent's last actions via `summarize_transcript_tail`, ASCII-only).
- Default `retry_on = "transient"` (3 attempts, backoff 5s/15s); `--retry-on off` restores stop-on-first-failure. `is_transient_agent_failure` retries `RunFailureKind.RUN` when `result_detail` is empty (SDK infra blip); run errors with SDK detail (billing, agent failure reports) are never retried.
