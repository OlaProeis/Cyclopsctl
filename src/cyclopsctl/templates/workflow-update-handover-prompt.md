...
**Project root for all `cyclopsctl tasks` commands:** `{{PROJECT_ROOT}}` (`--project-root {{PROJECT_ROOT}}`).
...
```bash
cyclopsctl tasks set-status --id=<current-task-id> --status=done --project-root {{PROJECT_ROOT}}
```
...
```bash
cyclopsctl tasks list pending --format json --project-root {{PROJECT_ROOT}}
```

From the pending list, pick the task with the **smallest numeric id**. Load full details:

```bash
cyclopsctl tasks show <next-task-id> --format json --project-root {{PROJECT_ROOT}}
```
