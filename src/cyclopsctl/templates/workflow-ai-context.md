...
- Run `{{TEST_CMD}}` after code changes to verify tests pass.
...
- **DO:** Run `{{TEST_CMD}}` before finishing; meet the task test strategy.
...
- **DO:** Use `cyclopsctl tasks list pending --project-root {{PROJECT_ROOT}}` and pick the **lowest numeric parent id** for the next handover — not `cyclopsctl tasks next` (priority can skip ahead).
...
- **DO:** Use `cyclopsctl tasks` with `--project-root {{PROJECT_ROOT}}` for all task commands (see Environment in the handover).
